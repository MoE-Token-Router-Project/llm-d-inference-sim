...
For a detailed explanation of how the simulator models inference time and what each latency parameter does, see [Latency Simulation](docs/latency-simulation.md). For suggested values for each parameter and ready-to-use YAML profiles, see [Latency Reference Tables and Profiles](docs/latency-profiles.md). For simulated multi-GPU Mixture-of-Experts routing, see [MoE Expert-Parallel Routing Simulation](docs/moe-routing.md).
