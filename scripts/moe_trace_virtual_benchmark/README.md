...
The serving loop matches the reference `moe_all_files/serve_models.py::serve_trace` structure while preserving the trace generator's decode semantics:

1. Every active decode sequence contributes one token to the next forward.
2. The remaining token budget is filled with stable-order prefill chunks.
3. The exact recorded logical expert routes are aggregated into one `[layer][expert]` workload.
4. Split, Concentrate, or Heuristic maps those assignments to the configured physical replicas.
5. All EP GPUs execute one shared forward and advance together.
6. For `N` visible output tokens, only `N-1` decode forwards are charged because the first output is produced by prefill.

For the controlled experiment in `moe_all_files`, pass the same frozen D'Hondt placement used by real vLLM:
