...
func ParseCommandParamsAndLoadConfig() (*Configuration, error) {
	...
	f.BoolVar(&config.EnableMoE, "enable-moe", config.EnableMoE, "Enable expert-parallel MoE latency simulation inside each data-parallel rank")
	f.IntVar(&config.MoEExpertParallelSize, "moe-expert-parallel-size", config.MoEExpertParallelSize, "Number of simulated GPUs hosting MoE expert replicas")
	f.IntVar(&config.MoENumExperts, "moe-num-experts", config.MoENumExperts, "Number of logical experts per MoE layer")
	f.IntVar(&config.MoEPhysicalExpertSlots, "moe-physical-expert-slots", config.MoEPhysicalExpertSlots, "Total number of physical expert replicas across expert-parallel GPUs")
	...
	f.IntVar(&config.MoEBytesPerElement, "moe-bytes-per-element", config.MoEBytesPerElement, "Tensor element width in bytes, for example 2 for BF16")
	f.Float64Var(&config.MoEGPUFlops, "moe-gpu-flops", config.MoEGPUFlops, "Simulated GPU peak throughput in FLOP/s")
	f.Float64Var(&config.MoEGPUMemoryBandwidth, "moe-gpu-memory-bandwidth", config.MoEGPUMemoryBandwidth, "Simulated GPU memory bandwidth in bytes/s")
	f.Float64Var(&config.MoEInterconnectBandwidth, "moe-interconnect-bandwidth", config.MoEInterconnectBandwidth, "Per-GPU expert-parallel interconnect bandwidth in bytes/s")
	f.DurationVar(&config.MoEInterconnectLatency, "moe-interconnect-latency", config.MoEInterconnectLatency, "One-way latency for each simulated expert all-to-all phase")
